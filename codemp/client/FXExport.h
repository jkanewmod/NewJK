...
void FX_PlayEffect( const char *file, vec3_t org, vec3_t fwd, int vol, int rad, int chan = -1 );		// builds arbitrary perp. right vector, does a cross product to define up

void FX_PlayEffectID( int id, vec3_t org, vec3_t fwd, int vol, int rad, qboolean isPortal = qfalse, int chan = -1 );		// builds arbitrary perp. right vector, does a cross product to define up
void FX_PlayEntityEffectID( int id, vec3_t org, matrix3_t axis, const int boltInfo, const int entNum, int vol, int rad, int chan = -1 );
