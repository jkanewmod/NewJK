	...
	switch(cinTable[currentHandle].roq_id)
	{
		...
		case	ZA_SOUND_MONO:
			if (!cinTable[currentHandle].silent) {
                ...
                S_RawSamples( ssize, 22050, 2, 1, (byte *)sbuf, s_volume->value*s_volumeMaster->value, 1 );
			}
			break;
		case	ZA_SOUND_STEREO:
			if (!cinTable[currentHandle].silent) {
                ...
                S_RawSamples( ssize, 22050, 2, 2, (byte *)sbuf, s_volume->value* s_volumeMaster->value, 1 );
			}
			break;
		case	ROQ_QUAD_INFO:
			if (cinTable[currentHandle].numQuads == -1) {
